        ...
        <div class="flex items-center justify-between pr-2 {{ $indent }} min-w-0 overflow-hidden">
            <div class="flex items-center gap-2 min-w-0 flex-1">
                ...
                <div class="flex items-center gap-1 min-w-0 flex-1 {{ (!$esRecurso || $esComposicion) ? 'cursor-pointer' : '' }}"
                    ...
                    @endif
                    @if(!$esRecurso && $nodo->depends_on_id && $nodo->dependeDe)
                        <span class="shrink-0 text-[9px] font-bold text-orange-300 bg-orange-500/10 border border-orange-500/20 rounded px-1 leading-4 whitespace-nowrap" title="Depende de: {{ $nodo->dependeDe->nombre }}">
                            → {{ Str::limit($nodo->dependeDe->nombre, 14) }}
                        </span>
                    @endif
                </div>
                {{-- Drag handle --}}
                ...
                @if(!$esRecurso)
                <button wire:click.stop="abrirModalDependencia({{ $nodo->id }})"
                    title="Dependencia Gantt"
                    class="w-5 h-5 flex items-center justify-center rounded transition
                        {{ $nodo->depends_on_id ? 'bg-orange-500/30 text-orange-300 hover:bg-orange-500/50' : 'bg-white/5 text-gray-600 hover:bg-white/10 hover:text-gray-300' }}">
                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
