<div class="min-h-screen bg-white dark:bg-[#090909] text-black dark:text-white p-6 space-y-6">
    ...
    <nav class="border-b border-gray-200 dark:border-white/5 bg-white dark:bg-[#0d0d0d]">
        ...
        <div class="flex items-center px-3 py-1.5 gap-3">
            ...
            <div class="min-w-0">
                ...
                <p class="text-[10px] text-gray-600 dark:text-gray-600 uppercase tracking-widest font-bold">ESTADÍSTICAS ▾</p>
            </div>
        </div>
            ...
            @foreach([
                ...
                <a href="{{ $tab['route'] }}"
                   class="flex items-center gap-1 px-2 py-1 rounded text-[11px] font-black uppercase tracking-wider transition-all whitespace-nowrap shrink-0
                       {{ $tab['active'] ? 'bg-gray-200 dark:bg-white text-black dark:text-black' : 'text-gray-500 dark:text-gray-500 hover:text-black dark:hover:text-white hover:bg-gray-100 dark:hover:bg-white/5' }}">
    ...
    {{-- CARDS SUPERIORES --}}
    <div class="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 2xl:grid-cols-6 gap-4">
        ...
        <div class="bg-gray-100 dark:bg-[#111] border border-gray-200 dark:border-gray-800/50 rounded-2xl p-5">
            <div class="flex items-center gap-2 mb-3">
                ...
                <p class="text-xs text-gray-500 font-black uppercase tracking-[0.15em]">Carga Social</p>
            </div>
            @if($stats['pctCS'] > 0)
                <p class="text-2xl font-black text-green-600 dark:text-green-400 tracking-tighter">USD {{ number_format($stats['cargaSocialTotal'], 0, ',', '.') }}</p>
                <p class="text-xs text-gray-400 dark:text-gray-600 mt-1">{{ number_format($stats['pctCS'], 1) }}% sobre Mano de Obra</p>
            @else
                <p class="text-2xl font-black text-gray-400 dark:text-gray-600 tracking-tighter">—</p>
                <p class="text-xs text-gray-400 dark:text-gray-600 mt-1">Sin % de carga social</p>
            @endif
        </div>
