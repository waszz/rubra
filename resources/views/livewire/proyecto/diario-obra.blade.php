<div class="min-h-screen bg-[#0a0a0a]">
    ...
    <div class="p-6 max-w-2xl mx-auto space-y-4">
        ...
        @forelse($rubros as $rubro)
            <div wire:key="diario-{{ $rubro['id'] }}"
                 class="bg-[#111] border border-white/5 rounded-2xl p-4 flex items-center justify-between hover:border-white/10 transition-all cursor-pointer group"
                 wire:click="abrirModal({{ $rubro['id'] }})">

                <div class="flex-1 min-w-0">
                    <p class="text-white font-black text-sm uppercase truncate">{{ $rubro['nombre'] }}</p>
                ...
                <svg class="w-4 h-4 text-gray-600 group-hover:text-white transition-colors ml-4 shrink-0"
    ...
    @if($mostrarModal)
        <div class="fixed inset-0 z-[90] flex items-end sm:items-center justify-center bg-black/80 backdrop-blur-md px-4 pb-4">
            <div class="w-full max-w-lg border border-white/10 rounded-2xl bg-[#0d0d0d] shadow-2xl overflow-hidden">
                ...
                <div class="flex items-center justify-between px-6 py-4 border-b border-white/5">
                    ...
                    <button wire:click="$set('mostrarModal', false)"
                        class="text-gray-600 hover:text-white transition-colors text-2xl leading-none">×</button>
                </div>
